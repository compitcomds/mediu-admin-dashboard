<template>
  <div>
    <div
      class="flex justify-between items-center bg-white shadow-sm p-4 mb-4 rounded-lg"
    >
      ...
      <div class="flex space-x-2 lg:space-x-4 items-center">
        ...
        >
          <input
            ...
            class="bg-transparent focus:outline-none text-sm py-2 w-48 lg:w-64"
            @input="performSearch"
          />
          <button
            @click="clearSearch"
...
// Reactive state
const predefinedTabs = ["All", "Active", "Draft", "Archived"];
...
const addFilter = () => {
  if (newFilterName.value.trim() !== "") {
    filters.push(newFilterName.value.trim());
    closeModal();
  }
};
...
// Emit function (to emit events from the setup)
