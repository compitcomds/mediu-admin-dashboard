<template>
  <div class="flex flex-col h-screen py-10 lg:py-5">
    ...
    <div class="lg:ml-64 flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 mt-4 bg-gray-100">
      ...
      <!-- Header -->
      <div class="flex flex-col lg:flex-row justify-between items-center space-y-4 mb-4">
        <h1 class="text-2xl text-[#28574e] font-semibold">Customers</h1>
        <div class="flex items-center sm:mb-0 bg-slate-200 lg:w-1/3 p-3 shadow-md rounded-md text-[#28574e]">
          <span class="mr-2"><span class="font-bold">{{ customers.length }}</span> customers</span>
        ...
        <table class="min-w-full table-auto bg-white border rounded">
          ...
          <tbody>
            <tr v-for="customer in filteredCustomers" :key="customer.id" class="border-b">
              <td class="px-4 py-2">
                <nuxt-link :to="`/customers/${customer.id}`" class="text-blue-600 hover:underline">
                  {{ customer.name }}
                </nuxt-link>
              </td>
...
export default {
  data() {
    return {
      customers: [
        { id: 1, name: 'Darshan Jain', emailSubscription: 'Not subscribed', location: '', orders: 2, amountSpent: 10153.9 },
        { id: 2, name: 'Compitcom Digital Solutions', emailSubscription: 'Not subscribed', location: '', orders: 0, amountSpent: 0 },
        { id: 3, name: 'Vikas Maur', emailSubscription: 'Not subscribed', location: '', orders: 0, amountSpent: 0 },
        { id: 4, name: 'Karine Ruby', emailSubscription: 'Not subscribed', location: '', orders: 0, amountSpent: 0 },
        { id: 5, name: 'Karine Ruby', emailSubscription: 'Not subscribed', location: '', orders: 0, amountSpent: 0 },
      ],
      searchQuery: '',
    };
  },
