<template>
  <div
    v-if="customer"
    class="flex flex-col lg:flex-row bg-gray-100 min-h-screen lg:space-x-2"
  >
    <!-- Main Content Area -->
    <div class="flex-1 lg:w-3/4 bg-white p-6 rounded-lg shadow-md">
      ...
      <div class="flex flex-col justify-between items-start mb-4">
        ...
        <div
          class="flex items-center justify-between w-full my-6 border bg-slate-100 rounded-md shadow"
        >
      ...
      <div class="mt-6 bg-gray-50 p-4 rounded-lg">
        ...
        <div class="border rounded-lg p-4">
          <div class="flex justify-between mb-4">
            <div>
              <p class="font-bold text-blue-600">
                Order #{{ customer.lastOrder.id }}
              </p>
              <p class="text-gray-500">
                {{ customer.lastOrder.date }} at
                {{ customer.lastOrder.time }} from
                {{ customer.lastOrder.history }}
              </p>
            </div>
          ...
          <div
            v-for="item in customer.lastOrder.items"
            :key="item.id"
          ...
          >
            <div class="flex items-center">
              ...
              <div class="flex flex-col">
                <span>{{ item.name }}</span>
                <p
                  class="bg-slate-200 rounded-2xl text-sm flex items-center justify-center h-6 w-6"
                >
                  {{ item.size }}
                </p>
              </div>
          ...
          <div class="flex justify-between items-center">
            <nuxt-link to="/customers" class="text-blue-500"
              >View all orders</nuxt-link
            >
            <button
      ...
      <div class="bg-white p-4 rounded-lg shadow-md mb-4">
        ...
        <div class="flex items-center">
          <a
            ...
            class="text-blue-600 mb-2 flex-1"
            >{{ customer.email }}</a
          >
          <button
          ...
          >
            ...
            >
              ...
              />
              <path
                d="M3 7a2 2 0 00-2 2v6a2 2 0 002 2h6a2 2 0 002-2v-6a2 2 0 00-2-2H3z"
              />
            </svg>
          </button>
        </div>
...
export default {
  ...
  computed: {
    ...
    lastOrderTotalAmount() {
      return this.customer.lastOrder.items.reduce(
  ...
  methods: {
            ...
            items: [
              {
                ...
                quantity: 2,
                price: 2000,
                imageUrl: "https://via.placeholder.com/40?text=Blowshine",
              },
              {
                id: 1,
                name: "blowshine",
          ...
          timeline: [
            {
              id: 1,
              message:
                "Order Confirmation email for order #1004 sent to this customer.",
              time: "7:05 AM",
            },
            {
              id: 2,
              message:
                "You created order #1004 for this customer from draft order #D11.",
              time: "7:04 AM",
            },
            {
              id: 3,
              message:
                "Order Confirmation email for order #1003 sent to this customer.",
              time: "6:20 AM",
            },
            {
              id: 4,
              message:
                "You created order #1003 for this customer from draft order #D12.",
              time: "6:20 AM",
            },
            {
              id: 5,
              message: "Headless created this customer.",
              time: "5:14 AM",
            },
          ],
        },
    ...
    deleteMessage(id) {
      this.customer.timeline = this.customer.timeline.filter(
        (event) => event.id !== id
      );
    },
  },
