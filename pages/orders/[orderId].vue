<template>
  <div class="flex flex-col min-h-screen py-10 px-4 lg:px-20 bg-gray-100">
    <!-- Background and padding adjusted for better aesthetics -->

    <div class="max-w-4xl mx-auto bg-white shadow-lg rounded-lg p-6">
      <!-- Center content on larger screens and add shadow for card effect -->

      <!-- Display order status -->
      <h1 class="text-2xl lg:text-3xl font-semibold text-gray-800 mb-6 text-center">
        Order Status: {{ order ? order.status : "Loading..." }}
      </h1>

      <!-- Order Details Section -->
      <div v-if="order" class="order-details">
        <h2 class="text-xl font-semibold text-gray-700 mb-4">Order ID: {{ order.id }}</h2>
        <p class="text-lg text-gray-600 mb-2">Customer Name: {{ order.name }}</p>
        <p class="text-lg text-gray-600 mb-2">
          Subtotal: {{ order.current_subtotal_price }}
        </p>
        <p class="text-lg text-gray-600 mb-2">Taxes CGST: {{ order.total_tax }}</p>
        <p class="text-lg text-gray-600 mb-2">
          Total Discount: {{ order.total_discounts }}
        </p>
        <p class="text-lg text-gray-600 mb-2">
        ...
        <p class="text-lg font-semibold text-gray-700 mt-4">
          Total Amount: {{ order.total_line_items_price }}
        </p>

        <button type="button" @click="confirmOrder" class="delete-button">
          Confirm Order
        </button>
      </div>
      ...
      <div v-if="error.length" class="error-message mt-6">
        <p
          ...
          :key="index"
          class="text-red-600 text-sm lg:text-base"
        >
          {{ err }}
        </p>
      </div>
    ...
    async confirmOrder() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          this.order.fulfillment_status = "fulfilled";
        } else {
          console.error("Fulfillment error response:", data); // Log the error response
          this.error = data.error || "Failed to fulfill the order";
        }
      } catch (err) {
        console.error("Error fulfilling the order:", err); // Log the error
        this.error.push("Error fulfilling the order.");
      }
    },
  },
};
